...
import com.example.onelook.common.util.adapters.SelectableOvalNumber
...
class AddEditSupplementViewModel @Inject constructor(
    ...
    private fun createOrUpdateSupplement() = viewModelScope.launch {
        ...
        val supplementEntity = SupplementEntity(
        ...
        if (_supplement.value == null)
            supplementRepository.createSupplement(supplementEntity).collect { result ->
                if (result is Resource.Success) {
                    _addSupplementEvent.emit(
                        AddSupplementEvent.NavigateBackAfterSupplementAdded(supplementEntity.name.capital)
                    )
                    _isLoading.emit(false)
                }
            }
        else
            supplementRepository.updateSupplement(supplementEntity).collect { result ->
                if (result is Resource.Success) {
                    _addSupplementEvent.emit(
                        AddSupplementEvent.NavigateBackAfterSupplementUpdated(supplementEntity.name.capital)
                    )
                    _isLoading.emit(false)
                }
            }
